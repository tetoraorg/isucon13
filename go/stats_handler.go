...
type LivestreamRankingEntry struct {
	...
	Score        int64         `db:"-"`
	TmpScore     sql.NullInt64 `db:"tmp_score"`
}
...
func getLivestreamStatisticsHandler(c echo.Context) error {
	...
	if err := tx.SelectContext(ctx, &ranking,
		"SELECT r.livestream_id AS livestream_id, COUNT(r.id) + IFNULL(SUM(lc.tip), 0) AS tmp_score FROM livestreams l LEFT JOIN reactions r ON l.id = r.livestream_id LEFT JOIN livecomments lc ON l.id = lc.livestream_id GROUP BY r.livestream_id ORDER BY tmp_score DESC",
	); err != nil {
